...
class IncrementalParser:
    ...
    def _parse(self):
        self.ctx = ParserCtx.SMT
        self.smts = []
        self.idx = 0
        self._inc_char(0)
        while self._read_smt():
            pass

    def _update_char(self):
        self.char = self.src[self.idx]

    def _inc_char(self, by=1):
        self.idx += 1
        self._update_char()
        return self.char

    def _peek(self, by=1):
        return self.src[self.idx + by]

    def _read_smt(self):
        if self.ctx == ParserCtx.SMT:
    ...
    def _read_identifier(self):
        self.result = ''
        while True:
            next_char = self._inc_char(1)
            if not self._is_ident_char(next_char):
                break
            self.result += next_char
        self._inc_char(-1)
        return self.result or None

    def _is_ident_char(self, c):
        return 'a' <= c.lower() <= 'z' or c in '_$'
